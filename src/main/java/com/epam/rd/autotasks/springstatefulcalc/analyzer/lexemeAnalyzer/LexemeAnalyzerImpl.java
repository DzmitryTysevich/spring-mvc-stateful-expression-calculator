package com.epam.rd.autotasks.springstatefulcalc.analyzer.lexemeAnalyzer;
 ...
 */
public class LexemeAnalyzerImpl implements Analyzer {
        ...
        while (position < expression.length()) {
            ...
            switch (character) {
                ...
                case DIVIDE_CHAR:
                    ...
                    break;
                default:
                    if (isCertainCharacters(character)) { //number parse
                        ...
                        } while (isCertainCharacters(character));
                        lexemes.add(new Lexeme(LexemeType.NUMBER, sb.toString()));
                    } else {
                        if (character != SPACE_CHAR) {
                            throw new ResponseStatusException(HttpStatus.CONFLICT, UNEXPECTED_CHARACTER + character);
                        }
                        position++;
                    }
                    break;
    ...
    private boolean isCertainCharacters(char character) {
        return (character <= NINE_CHAR && character >= ZERO_CHAR);
    }
